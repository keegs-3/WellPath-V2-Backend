...
# Don't convert "None" string to NaN - keep it as a string
patient_survey = pd.read_csv(os.path.join(base_dir, "data", "synthetic_patient_survey.csv"), keep_default_na=False, na_values=[''])
...
def coping_score(answer_str, coping_weights, stress_level_ans, freq_ans):
    ...
    # Debug for patient 0cc07ac6
    if "None" in responses or "none" in str(answer_str).lower():
        print(f"DEBUG coping_score: answer_str='{answer_str}', responses={responses}, has_none={has_none}, high_stress={high_stress}")
...
base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
survey_output_dir = os.path.join(base_dir, "outputs/scores/survey")
    ...
    for qid, config in QUESTION_CONFIG.items():
        ...
        # Handle complex calculations
            ...
            if str(answer).strip() == "Yes":
                # Use complex calculation for diagnosed sleep apnea patients
                diagnosis = str(row.get("4.28", "")).strip()
                severity = str(row.get("4.30", "")).strip()
                time_since = str(row.get("4.29", "")).strip()
                treatments = str(row.get("4.31", "")).strip()
                improvement = str(row.get("4.32", "")).strip()
                score = calc_sleep_apnea_score(diagnosis, severity, time_since, treatments, improvement)
            else:
                # Use direct response scores for "No" and "I suspect..."
...
print("✓ Per-question raw, weighted, and max scores saved to outputs/scores/survey/per_question_scores_full_weighted.csv")
...
print("✓ Gap analysis saved to outputs/scores/survey/question_gap_analysis.csv")
...
print("✓ Final pillar scores saved to outputs/scores/survey/synthetic_patient_pillar_scores_survey_with_max_pct.csv")
