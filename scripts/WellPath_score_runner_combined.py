...
def process_complex_survey_calculations(patient_record, survey_raw_row, pillar_names, pillar_weights):
    ...
    # 1. PROTEIN INTAKE (Question 2.11) - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes:
    # - 2.11_Nutrition_weighted (Healthful Nutrition pillar)
    # - 2.11_Movement_weighted (Movement + Exercise pillar)
    # Adding them here would DOUBLE-COUNT these scores!
    pass
    ...
    # 3. MOVEMENT/EXERCISE (Questions 3.04-3.11) - REMOVED: Already handled by per-question CSV
    # The per-question CSV from wellpath_score_runner_survey_v2.py already includes:
    # - Cardio_Movement_weighted (from 3.04 + 3.08)
    # - Strength_Movement_weighted (from 3.05 + 3.09)
    # - Flexibility_Movement_weighted (from 3.06 + 3.10)
    # - HIIT_Movement_weighted (from 3.07 + 3.11)
    # Adding them here would DOUBLE-COUNT these scores!
    pass

    # 4. SLEEP ISSUES (Questions 4.12-4.19) - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes:
    # - 4.12_Sleep_weighted (Restorative Sleep pillar)
    # - 4.12_CoreCare_weighted (Core Care pillar)
    # - 4.12_Movement_weighted (Movement + Exercise pillar)
    # Adding them here would DOUBLE-COUNT these scores across all 3 pillars!
    pass

    # 5. SLEEP APNEA COMPLEX CALCULATION (Questions 4.28-4.33) - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes:
    # - 4.28_Sleep_weighted (Restorative Sleep pillar)
    # - 4.28_CoreCare_weighted (Core Care pillar)
    # Adding them here would DOUBLE-COUNT these scores on both pillars!
    pass

    # 6. SLEEP HYGIENE PROTOCOLS (Question 4.07) - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes:
    # - 4.07_Sleep_weighted (Restorative Sleep pillar)
    # Adding it here would DOUBLE-COUNT this score!
    pass

    # 7. COGNITIVE ACTIVITIES (Question 5.08) - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes:
    # - 5.08_Cognitive_weighted (Cognitive Health pillar)
    # Adding it here would DOUBLE-COUNT this score!
    pass

    # 8. STRESS ASSESSMENT (Questions 6.01 & 6.02) - PARTIALLY REMOVED
    # The per-question CSV already includes:
    # - 6.02_Stress_weighted (Stress Management pillar)
    # However, 6.01 appears to NOT be in the per-question CSV, so we need to check if
    # this combined stress score (6.01 + 6.02) is being double-counted.
    # For now, REMOVING to avoid double-counting 6.02
    pass

    # 9. COPING SKILLS (Question 6.07) - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes:
    # - 6.07_Stress_weighted (Stress Management pillar)
    # Adding it here would DOUBLE-COUNT this score!
    pass

    # 10. SUBSTANCE USE SCORING - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes all 6 substance scores:
    # - Tobacco_CoreCare_weighted
    # - Alcohol_CoreCare_weighted
    # - Recreational Drugs_CoreCare_weighted
    # - Nicotine_CoreCare_weighted
    # - OTC Meds_CoreCare_weighted
    # - Other Substances_CoreCare_weighted
    # Adding them here would DOUBLE-COUNT all substance use scores!
    pass

    # 11. SCREENING GUIDELINES (Questions 10.01-10.08) - REMOVED: Already handled by per-question CSV
    # The per-question CSV already includes:
    # - 10.01_CoreCare_weighted (Dental)
    # - 10.02_CoreCare_weighted (Skin check)
    # - 10.03_CoreCare_weighted (Vision)
    # - 10.04_CoreCare_weighted (Colonoscopy)
    # - 10.05_CoreCare_weighted (Mammogram)
    # - 10.06_CoreCare_weighted (PAP)
    # - 10.08_CoreCare_weighted (PSA)
    # Note: 10.07 (DEXA) is NOT in per-question CSV, but keeping consistent by removing all
    # Adding them here would DOUBLE-COUNT all screening questions!
    pass
...
def calc_sleep_apnea_score_exact(diagnosis, severity, time_since, treatments, improvement):
    """
...
if __name__ == "__main__":
    ...
    if result is not None:
        ...
        print("="*80)
        print("Files created in outputs/scores/combined/:")
