...
def extract_actual_detailed_surveys(patient_row):
    ...
    exclude_question_ids = ['3.04', '3.05', '3.06', '3.07', '3.08', '3.09', '3.10', '3.11', '6.01', '6.02', '6.07', '4.13', '4.14', '4.15', '4.16', '4.17', '4.18', '4.28', '4.29', '4.30', '4.31', '4.32', '4.33']  # Exercise freq/dur, stress level/freq, coping methods, sleep frequencies, sleep apnea complex
...
def extract_complex_survey_calculations(patient_row, demographics):
    ...
    # 5. Sleep Issues Complex Calculation (4.12 issues + 4.13-4.18 frequencies)
    ...
    if sleep_issues_response:
        ...
        freq_responses = []
        for qid in ['4.13', '4.14', '4.15', '4.16', '4.17', '4.18']:
    ...
    # 6. Sleep Apnea Complex Calculation (4.28-4.33)
    sleep_apnea_diagnosis = patient_row.get('survey_4.28_Restorative Sleep_response', '')
    if sleep_apnea_diagnosis:
        apnea_responses = []
        apnea_responses.append(f"4.28 Diagnosis: {sleep_apnea_diagnosis}")
        
        # Collect all related responses using the correct column format
        for qid, label in [
            ('4.29', 'Time Since Diagnosis'),
            ('4.30', 'Severity'), 
            ('4.31', 'Current Treatments'),
            ('4.32', 'Treatment Improvement'),
            ('4.33', 'Willingness to Try Treatments')
        ]:
            # Use the correct column format found in comprehensive data
            col_name = f'survey_q_{qid}_response'
            if col_name in patient_row.index:
                response = patient_row.get(col_name, '')
                if response and str(response) not in ['', 'nan', 'None']:
                    apnea_responses.append(f"{qid} {label}: {response}")
                else:
                    apnea_responses.append(f"{qid} {label}: [no response]")
            else:
                apnea_responses.append(f"{qid} {label}: [not available]")
        
        apnea_detail = " | ".join(apnea_responses)
        complex_calculations['sleep_apnea'] = {
            'question_id': '4.28',
            'type': 'Sleep Apnea Complex Calculation (Sleep Quality-Focused)',
            'response': apnea_detail
        }
