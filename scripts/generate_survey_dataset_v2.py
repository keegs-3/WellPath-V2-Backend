...
def answer_2_13(patient, dietary_restrictions):
    """
    How often do you consume processed meat (such as bacon, sausages, hot dogs, ham, salami, pepperoni, deli meats, or canned meats)?
...
def answer_2_14(patient, red_meat_resp):
    """Would you consider decreasing your processed meat consumption in support of your longevity goals?"""
...
def answer_2_67(patient, dietary_restrictions):
    """
    How often do you consume red meat (such as beef, pork, lamb, veal, or goat)?
    - If vegan, vegetarian, or pescatarian: always 'Rarely or Never'
    - If only 'No Restrictions', or only stackable restrictions (e.g. intermittent fasting, paleo): use normal diet_quality logic
    """
    if isinstance(dietary_restrictions, str):
...
def answer_2_68(patient, red_meat_resp):
    """Would you consider decreasing your red meat consumption in support of your longevity goals?"""
    if red_meat_resp not in ["3-4 times per week", "5 or more times per week"]:
        return ""
    options = [
        "Yes - actively working on it",
...
def answer_2_19(patient, dietary_restrictions):
    """
    How many total servings of fruit do you consume daily?
...
def answer_2_20(patient, fv_resp):
    """Would you consider increasing your daily fruit consumption in support of your longevity goals?"""
...
def answer_2_65(patient, dietary_restrictions):
    """
    How many total servings of vegetables do you consume daily??
    - Vegan or Vegetarian: almost always '3-4' or '5 or more'
    - Else: weighted random
    """
    if isinstance(dietary_restrictions, str):
...
def answer_2_66(patient, fv_resp):
    """Would you consider increasing your daily vegetable consumption in support of your longevity goals?"""
    if fv_resp not in ["0", "1-2"]:
        return ""
    options = [
        "Yes - actively working on it",
...
def answer_3_20(patient, exercise_types):
    ...
    if "Flexibility/mobility (e.g. yoga, stretching)" in exercise_types:
        return ""
    options = [
        "Professional guidance (e.g., personal trainer)",
        "Online tutorials or videos",
...
def answer_3_28(patient, exercise_types):
    """What are the primary reasons that flexibility/mobility training is not a part of your routine (select all that apply)?"""
    if "Flexibility/mobility (e.g. yoga, stretching)" in exercise_types:
        return ""
    options = [
        "Lack of knowledge about flexibility/mobility training",
        "Lack of time in my current routine",
        "Preference for other types of exercise",
        "Perception that it is not necessary for my goals",
        "Find it boring or unengaging",
        "Don't see immediate results or benefits",
        "Physical limitations or medical conditions",
        "Forget to include it in my routine",
        "Don't know how often or how long to stretch",
        "Other (please specify)"
    ]
    return "|".join(random.sample(options, k=random.randint(1, 3)))

def answer_3_29(patient, exercise_types):
    """Would you consider adding flexibility/mobility training in support of your longevity goals?"""
    if "Flexibility/mobility (e.g. yoga, stretching)" in exercise_types:
        return ""
    options = [
        "Yes - open to trying",
...
def answer_3_30(patient, exercise_types):
    """What resources would help you start or optimize a flexibility/mobility training routine (select all that apply)?"""
    if "High-intensity interval training (HIIT)" in exercise_types:
        return ""
    options = [
        "Professional guidance (e.g., physical therapist, trainer)",
        "Online tutorials or videos",
        "Detailed stretching/mobility plans",
        "Access to yoga or stretching classes",
        "Support from friends or workout partners",
        "Home stretching equipment (e.g., yoga mat, foam roller)",
        "Educational articles or books",
        "None",
        "Other (please specify)"
    ]
...
def answer_4_12(patient):
    ...
    options = [
        ...
        "Frequent nightmares", "Restless legs", "Snoring", "Other (please specify)"
    ]
...
def answer_4_28(patient):
    """Have you ever been diagnosed with sleep apnea?"""
    options = [
        "Yes", 
        "No", 
        "I suspect I may have it but haven't been diagnosed"
    ]
    # Higher chance of "No", moderate chance of suspicion, lower chance of diagnosis
    weights = [0.15, 0.70, 0.15]
    return random.choices(options, weights=weights)[0]

def answer_4_29(patient, apnea_diagnosis):
    """How long has it been since your sleep apnea diagnosis?"""
    if apnea_diagnosis != "Yes":
        return ""
    options = [
        "Less than 6 months",
        "6 months to 1 year", 
...
def answer_4_30(patient, apnea_diagnosis):
    """What was the severity of your sleep apnea at the time of diagnosis?"""
    if apnea_diagnosis != "Yes":
        return ""
    options = [
        ...
        "I don't know/wasn't told the severity"
    ]
    # Most common distribution: moderate > mild > severe > unknown
    weights = [0.35, 0.40, 0.20, 0.05]
    return random.choices(options, weights=weights)[0]

def answer_4_31(patient, apnea_diagnosis):
    """Which of the following treatments are you currently using for your sleep apnea? (select all that apply)"""
    if apnea_diagnosis != "Yes":
        return []
    options = [
        "CPAP machine",
        "BiPAP machine", 
        "Oral appliance/dental device",
        "Positional therapy (sleeping position changes)",
        "Weight loss program",
        "Surgery (e.g., UPPP, septoplasty)",
        "Lifestyle modifications (diet, exercise, avoiding alcohol)",
        "No treatment currently",
        "Other (please specify)"
    ]
    
    # Most common: CPAP, lifestyle modifications, weight loss
    if random.random() < 0.15:  # 15% not currently treating
        return ["No treatment currently"]
    
    # Build treatment list
    treatments = []
    if random.random() < 0.60:  # 60% use CPAP
        treatments.append("CPAP machine")
    if random.random() < 0.10:  # 10% use BiPAP  
        treatments.append("BiPAP machine")
    if random.random() < 0.25:  # 25% use oral appliance
        treatments.append("Oral appliance/dental device")
    if random.random() < 0.30:  # 30% use positional therapy
        treatments.append("Positional therapy (sleeping position changes)")
    if random.random() < 0.45:  # 45% doing weight loss
        treatments.append("Weight loss program")
    if random.random() < 0.05:  # 5% had surgery
        treatments.append("Surgery (e.g., UPPP, septoplasty)")
    if random.random() < 0.55:  # 55% doing lifestyle modifications
        treatments.append("Lifestyle modifications (diet, exercise, avoiding alcohol)")
    if random.random() < 0.08:  # 8% other treatments
        treatments.append("Other (please specify)")
    
    return treatments if treatments else ["No treatment currently"]

def answer_4_32(patient, apnea_treatments):
    """How much has your current sleep apnea treatment improved your symptoms?"""
    if not apnea_treatments or "No treatment currently" in apnea_treatments:
        return ""
    options = [
    ...
    # Better outcomes with CPAP/BiPAP, moderate with other treatments
    if "CPAP machine" in apnea_treatments or "BiPAP machine" in apnea_treatments:
...
def answer_4_33(patient, apnea_diagnosis):
    """Which of the following sleep apnea treatments would you be willing to try? (select all that apply)"""
    if apnea_diagnosis == "Yes":  # Already diagnosed - skip this question
        return []
    if apnea_diagnosis == "No":   # No diagnosis, no suspicion - less willing
        if random.random() < 0.70:  # 70% say "None of these"
            return ["None of these"]
    
    # For those who suspect they have it or answered "No" but willing to try
    options = [
        "CPAP machine",
        "BiPAP machine",
        "Oral appliance/dental device", 
    ...
    willing = []
    if random.random() < 0.25:  # 25% willing to try CPAP
        willing.append("CPAP machine")
    if random.random() < 0.15:  # 15% willing to try BiPAP
        willing.append("BiPAP machine")
    if random.random() < 0.40:  # 40% willing to try oral appliance
        willing.append("Oral appliance/dental device")
    if random.random() < 0.50:  # 50% willing to try positional therapy
        willing.append("Positional therapy (sleeping position changes)")
    if random.random() < 0.65:  # 65% willing to try weight loss
        willing.append("Weight loss program")
    if random.random() < 0.10:  # 10% willing to try surgery
        willing.append("Surgery (e.g., UPPP, septoplasty)")
    if random.random() < 0.75:  # 75% willing to try lifestyle modifications
        willing.append("Lifestyle modifications (diet, exercise, avoiding alcohol)")
    if random.random() < 0.05:  # 5% other
        willing.append("Other (please specify)")
    
    return willing if willing else ["None of these"]
...
def generate_survey_responses(profile_csv, out_csv=None, seed=42):
    ...
    for _, row in df_profiles.iterrows():
        ...
        processed_meat_resp = answer_2_13(patient, dietary_restrictions)
        ans_2_14 = answer_2_14(patient, processed_meat_resp)
        red_meat_resp = answer_2_67(patient, dietary_restrictions)
        ans_2_68 = answer_2_68(patient, red_meat_resp)
        ...
        fruit_resp = answer_2_19(patient, dietary_restrictions)
        ans_2_20 = answer_2_20(patient, fruit_resp)
        veg_resp = answer_2_65(patient, dietary_restrictions)
        ans_2_66 = answer_2_66(patient, veg_resp)
        ...
        apnea_diagnosis = answer_4_28(patient)
        apnea_time_since = answer_4_29(patient, apnea_diagnosis)
        apnea_severity = answer_4_30(patient, apnea_diagnosis)
        apnea_treatments = answer_4_31(patient, apnea_diagnosis)
        ...
        row_data = {
            ...
            '3.28': answer_3_28(patient, exercise_types),
            '3.29': answer_3_29(patient, exercise_types),
            '3.30': answer_3_30(patient, exercise_types),
            '3.21': steps_resp,
            ...
            '4.27': retry_device,
            '4.28': apnea_diagnosis,
            '4.29': apnea_time_since,
            '4.30': apnea_severity,
            '4.31': "|".join(apnea_treatments),
            '4.32': answer_4_32(patient, apnea_treatments),
            '4.33': "|".join(answer_4_33(patient, apnea_diagnosis)),
            '5.01': cognitive_func,
            '5.02': cog_concerns,
