...
class StatisticalImpactScorer:
    ...
    def __init__(self, recommendations_file: str, markers_file: str, comprehensive_file: str):
        ...
        self.PILLAR_WEIGHTS = {
            'Healthful Nutrition': 1/7,
            'Movement + Exercise': 1/7,
            'Restorative Sleep': 1/7,
            'Stress Management': 1/7,
            'Cognitive Health': 1/7,
            'Connection + Purpose': 1/7,
            'Core Care': 1/7
        }
...
def get_default_file_paths(base_dir: str) -> Dict[str, str]:
    ...
    Output files go to outputs/reports/impact_scores folder.
    """
    # Input directory - where the combined scoring data is located
    input_dir = os.path.join(base_dir, "outputs/scores/combined")
    
    # Output directory - where impact scoring results will be saved
    output_dir = os.path.join(base_dir, "outputs/reports/impact_scores")
