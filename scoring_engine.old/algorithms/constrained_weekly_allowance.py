...
class ConstrainedWeeklyAllowanceAlgorithm:
    ...
    def calculate_dual_progress(self, daily_values: List[Union[float, int]], current_day: int) -> Dict[str, float]:
        """
        Calculate dual progress metrics for constrained weekly allowance goals.
        
        For dual constraint goals (weekly allowance + max days):
        - Progress: 100% if both constraints met, 0% if either violated
        - Potential: Whether both constraints can still be satisfied
        """
        if current_day > len(daily_values):
            current_day = len(daily_values)
            
        # Calculate current usage and days used so far
        current_usage = sum(daily_values[:current_day])
        days_used = sum(1 for value in daily_values[:current_day] if value > 0)
        weekly_allowance = self.config.weekly_allowance
        max_days = self.config.max_days_per_week
        
        # Check both constraints
        weekly_constraint_violated = current_usage > weekly_allowance
        days_constraint_violated = max_days is not None and days_used > max_days
        
        # Progress: 100% if all constraints satisfied, 0% if any violated
        if weekly_constraint_violated or days_constraint_violated:
            progress_towards_goal = 0.0
        else:
            # Both constraints satisfied - success
            progress_towards_goal = 100.0
            
        # Max potential: Can we still satisfy both constraints?
        remaining_days = len(daily_values) - current_day
        remaining_allowance = max(0, weekly_allowance - current_usage)
        
        # Check if we can still satisfy both constraints
        can_satisfy_weekly = current_usage <= weekly_allowance
        can_satisfy_days = max_days is None or days_used <= max_days
        
        if can_satisfy_weekly and can_satisfy_days:
        ...
        return {
            ...
            'weekly_constraint_violated': weekly_constraint_violated,
            'days_constraint_violated': days_constraint_violated
        }
...
def calculate_constrained_weekly_allowance_dual_progress(
    ...
    **kwargs
    ...
    """
    Calculate dual progress for constrained weekly allowance goals
    
    Args:
        daily_values: List of daily measured values
        current_day: Current day (1-7)
        weekly_allowance: Maximum weekly allowance
    ...
    config = ConstrainedWeeklyAllowanceConfig(
        weekly_allowance=weekly_allowance,
        unit=unit,
        rollover_enabled=kwargs.get('rollover_enabled', False),
        max_rollover_percentage=kwargs.get('max_rollover_percentage', 50.0),
        minimum_weekly_usage=kwargs.get('minimum_weekly_usage', 0.0),
        penalty_for_overage=kwargs.get('penalty_for_overage', 0.0),
        max_days_per_week=kwargs.get('max_days_per_week')
    )
