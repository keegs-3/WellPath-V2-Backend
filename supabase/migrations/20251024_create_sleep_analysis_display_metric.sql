...
-- =====================================================
-- Parent metric shows comprehensive sleep analysis:
-- - Time in Bed
-- - Time Asleep
...
-- Update existing sleep parent to be comprehensive sleep analysis
...
SET
  ...
  summary_stat_1_label = 'Time in Bed',
  summary_stat_2_label = 'Time Asleep',
  primary_unit = 'hours'
...
INSERT INTO parent_detail_sections (
  ...
  'sleep_stages_stacked',
  1,
  true
) ON CONFLICT (section_id, parent_metric_id) DO UPDATE SET
  section_name = EXCLUDED.section_name,
  section_chart_type_id = EXCLUDED.section_chart_type_id;

-- Create child display metrics for sleep stages
...
-- Link child metrics to aggregations for daily period
INSERT INTO parent_child_display_metrics_aggregations (
  ...
  -- Deep Sleep
  ('DISP_TOTAL_SLEEP_DURATION', 'DISP_DEEP_SLEEP', 'AGG_DEEP_SLEEP_DURATION', 'daily', 'SUM'),
  -- Core Sleep
  ('DISP_TOTAL_SLEEP_DURATION', 'DISP_CORE_SLEEP', 'AGG_CORE_SLEEP_DURATION', 'daily', 'SUM'),
  -- REM Sleep
  ('DISP_TOTAL_SLEEP_DURATION', 'DISP_REM_SLEEP', 'AGG_REM_SLEEP_DURATION', 'daily', 'SUM'),
  -- Total Sleep Duration (parent metric main value)
...
ON CONFLICT (parent_metric_id, child_metric_id, agg_metric_id, period_type, calculation_type_id)
DO NOTHING;

-- Add to display screen if not already there
INSERT INTO display_screens_display_metrics (
  ...
  'main',
  'top'
) ON CONFLICT (display_screen, display_metric) DO NOTHING;
...
Child Metrics (Sleep Stages):
  1. Deep Sleep (dark blue, bottom)
  2. Core Sleep (blue, middle)
  3. REM Sleep (cyan, upper)
  4. Awake (orange, top)

Section: SECTION_SLEEP_STAGES
Chart: sleep_stages_stacked
