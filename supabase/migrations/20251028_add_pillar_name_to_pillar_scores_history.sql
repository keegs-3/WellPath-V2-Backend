-- Add pillar_name to patient_pillar_scores_history and make it required
...
UPDATE patient_pillar_scores_history h
...
ALTER TABLE patient_pillar_scores_history
ADD CONSTRAINT patient_pillar_scores_history_pillar_name_fkey
FOREIGN KEY (pillar_name) REFERENCES pillars_base(pillar_name)
ON UPDATE CASCADE ON DELETE CASCADE;

-- Add index for pillar_name queries
CREATE INDEX IF NOT EXISTS idx_pillar_scores_history_pillar_name
ON patient_pillar_scores_history(pillar_name);

-- Add index for patient + pillar_name (common mobile query pattern)
CREATE INDEX IF NOT EXISTS idx_pillar_scores_history_patient_pillar_name
ON patient_pillar_scores_history(patient_id, pillar_name, calculated_at DESC);

-- Add comment
COMMENT ON COLUMN patient_pillar_scores_history.pillar_name IS
