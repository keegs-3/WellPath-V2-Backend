...
def process_complex_survey_calculations(patient_record, survey_raw_row, pillar_names, pillar_weights):
    ...
    # 5. SLEEP APNEA COMPLEX CALCULATION (Questions 4.28-4.33) - Using sleep quality-focused scoring
    diagnosis_response = survey_raw_row.get('4.28', '')
    if diagnosis_response and str(diagnosis_response) not in ['', 'nan', 'No response']:
        severity = survey_raw_row.get('4.30', '')
        time_since = survey_raw_row.get('4.29', '')
        treatments = survey_raw_row.get('4.31', '')
        improvement = survey_raw_row.get('4.32', '')
        
        raw_score = calc_sleep_apnea_score_exact(diagnosis_response, severity, time_since, treatments, improvement)
        
        # Apply to both Sleep and CoreCare pillars (same weights as in survey runner)
        sleep_weight = 7.0
        corecare_weight = 3.0
        
        # Sleep pillar
        sleep_weighted_score = raw_score * sleep_weight
        prefix = f"survey_4.28_Restorative Sleep"
        patient_record.update({
            f"{prefix}_response": f"Sleep Apnea: {diagnosis_response}",
            f"{prefix}_score": raw_score,
            f"{prefix}_weight": sleep_weight,
            f"{prefix}_weighted_score": sleep_weighted_score,
            f"{prefix}_max_weighted": sleep_weight,
            f"{prefix}_improvement_potential": sleep_weight - sleep_weighted_score
        })
        
        # CoreCare pillar  
        corecare_weighted_score = raw_score * corecare_weight
        prefix = f"survey_4.28_Core Care"
        patient_record.update({
            f"{prefix}_response": f"Sleep Apnea Management: {diagnosis_response}",
            f"{prefix}_score": raw_score,
            f"{prefix}_weight": corecare_weight,
            f"{prefix}_weighted_score": corecare_weighted_score,
            f"{prefix}_max_weighted": corecare_weight,
            f"{prefix}_improvement_potential": corecare_weight - corecare_weighted_score
        })

    # 6. SLEEP HYGIENE PROTOCOLS (Question 4.07) - Using your exact score_sleep_protocols logic
